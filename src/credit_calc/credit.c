...
Credit_output credit_calculator(Credit_params credit_params) {
  ...
  double monthly_rate = (credit_params.interest_rate / 100.0) / 12.0 * (30.81 / average_days_per_month);
...
//   printf("monthly_payment -> %lf | overpayment -> %lf | total_payout -> %lf\n", credit_result.monthly_payment, credit_result.overpayment, credit_result.total_payout);
//   printf("minpayment -> %lf | maxpayment -> %lf\n", credit_result.min_payment, credit_result.max_payment);
//   return 0;
// }