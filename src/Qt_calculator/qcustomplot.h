...
void QCPDataContainer<DataType>::add(const DataType &data) {
  if (isEmpty() ||
      !qcpLessThanSortKey<DataType>(data,
          *(constEnd() - 1)))  // quickly handle appends if new data key is greater or equal to existing ones
  {
    mData.append(data);
...
class QCP_LIB_DECL QCPAbstractPlottable : public QCPLayerable {
 ...
 public:
  ...
  virtual double selectTest(const QPointF &pos, bool onlySelectable,
      QVariant *details = nullptr) const Q_DECL_OVERRIDE = 0;  // actually introduced in QCPLayerable as non-pure, but
                                                               // we want to force reimplementation for plottables
...
ItemType *QCustomPlot::itemAt(const QPointF &pos, bool onlySelectable) const {
  ...
  foreach (QCPAbstractItem *item, mItems) {
    ...
    if (!currentItem ||
        (onlySelectable && !currentItem->selectable()))  // we could have also passed onlySelectable to the selectTest
...
class QCP_LIB_DECL QCPGraph : public QCPAbstractPlottable1D<QCPGraphData> {
 ...
 protected:
  ...
  QVector<QPair<QCPDataRange, QCPDataRange> > getOverlappingSegments(QVector<QCPDataRange> thisSegments,
      const QVector<QPointF> *thisData, QVector<QCPDataRange> otherSegments, const QVector<QPointF> *otherData) const;
...
class QCP_LIB_DECL QCPCurve : public QCPAbstractPlottable1D<QCPCurveData> {
 ...
 protected:
  ...
  QVector<QPointF> getOptimizedCornerPoints(int prevRegion, int currentRegion, double prevKey, double prevValue,
      double key, double value, double keyMin, double valueMax, double keyMax, double valueMin) const;
